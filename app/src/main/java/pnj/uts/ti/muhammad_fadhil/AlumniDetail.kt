...
class AlumniDetail : AppCompatActivity() {
    ...
    private fun loadRecordData(id: Long) {
        ...
        if (cursor.moveToFirst()) {
            etNim.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_NIM)))
            etName.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_NAME)))
            etPlaceOfBirth.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_PLACE_OF_BIRTH)))
            etDateOfBirth.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_DATE_OF_BIRTH)))
            etAddress.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_ADDRESS)))
            etReligion.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_RELIGION)))
            etPhone.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_PHONE)))
            etYearOfEntry.setText(cursor.getInt(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_YEAR_OF_ENTRY)).toString())
            etYearOfGraduation.setText(cursor.getInt(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_YEAR_OF_GRADUATION)).toString())
            etJob.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_JOB)))
            etPosition.setText(cursor.getString(cursor.getColumnIndexOrThrow(DBHelper.COLUMN_POSITION)))
        }
        cursor.close()
    }
